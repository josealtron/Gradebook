...
class Gr_Category(object):
    # --------------------------------------------------------------------------
    # initialization and overloaded operators
    
    def __init__(self, name, avg=None, parent=None, w=0):
        """
        Initializes Grade Category object. Gr_Category instances have the following
        data attributes:
            
            name - Name of the category, taken as a string parameter on intialization.
        ...
        """   
        try:
            assert type(name) == str and (isNum(avg) or avg is None)
            if avg != None:
                assert avg >= 0
            self.name = name
            self.avg = avg
            ...
            print ('Gr_Category __init__: invalid input')
    ...
    def force_avg(self, new_avg):
        ...
        """
        try:
            assert new_avg >= 0 and (isNum(new_avg))
            self.avg = new_avg
            self.manual_avg = True
            if new_avg > 100:
                print('Note: Average has been set to a number greater than 100.')
            if self.parent:
    ...
    def add_grade(self, name, grade, date=None):
        ...
        try:
            ...
            assert isNum(grade)
            assert grade >= 0
            assert (type(date) == tuple or date == None)
            if type(date) == tuple:
                assert len(date) == 3
...
class Gradebook(object):
        # ----------------------------------------------------------------------
        # Initalization and overloaded operators
